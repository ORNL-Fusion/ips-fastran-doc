...
#        print('Running vmeclauncher')
        ...
        #for calculation of input variables 
        ...
        if alpha_EP == 1:
            ...
            betalf = 0.0
            omgcya =0.0
        inv_norm=1.0/far3d_profile.density_norm
        nion_SI = peak_nion * inv_norm
        betf= 2.0 * mu0 * peak_pbeam_kPa*1e3 / (B0_e**2)
        va0 = B0_e / math.sqrt(uion_e * mu0 * nion_SI * m_p)
        ...
        for n in n_values:
            ...
            customized = (
                template.replace("<MM_LINE>", format_list("mm", mm_vals))
                        ...
                        .replace("<ALPHA_ON>", format_scalar("alpha_on",alphaon))
       ...
       #     dask_nodes = 1
        ...
        self.run_far3d_with_dask(run_dirs, str(far3d_exe), dask_nodes, timeid)

    def run_far3d_with_dask(self, run_dirs, far3d_exe, dask_nodes,timeid):
