...
class far3d_io_profile():
    def __init__(self,alpha_EP):
        ...
        if alpha_EP==1: 
            ...
            self.density_norm = 1.e-19
            self.key_map = {
                'rho'    : [0, 'Rho', '(norml_sqrt_toroid_flux)'],
                'q'      : [1, 'q', ''],
                'nbeam'  : [2, 'BeamIonDensity', '(10^13cm^-3)'],
                'nion'   : [3, 'IonDensity', '(10^13cm^-3)'],
                'ne'     : [4, 'ElecDensity', '(10^13cm^-3)'],
                'nz'     : [5, 'ImpurityDensity', '(10^13cm^-3)'],
    ...
    def from_state(self, f_instate, f_state):
        ...
        self['rho'] = rho
        self['q'] = ps['q_eq'][:]  
        self['nbeam'] = density_beam *self.density_norm
        self['nion'] = np.sum(ni, axis=0) * self.density_norm
        self['ne'] = ne  * self.density_norm
        self['nz'] = np.sum(nz, axis=0) * self.density_norm
        self['tbeam'] = tbeam  
        self['ti'] = ti 
        self['te'] = te     
        ...
        if self.alpha_EP==1:
            ...
            print(talpha)
            print(walpha)
            self['nalpha'] =  density_alpha * self.density_norm
            self['talpha'] = talpha 
        print('done')
